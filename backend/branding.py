...
def build_email_header_html(subtitle: str = "") -> str:
    ...
    subtitle_html = (
        f'<div style="font-size:14px;color:#6B7280;margin-top:4px">{subtitle}</div>'
        if subtitle
        else ""
    )
    logo_img = (
        f'<img src="{LOGO_DATA_URL}" alt="{BUSINESS_NAME}" '
        f'style="max-height:80px;max-width:260px;display:block;margin:0 auto 12px auto" />'
        if LOGO_DATA_URL
        else ""
    )
    return f"""
    <div style="text-align:center;padding:24px 16px 16px 16px;border-bottom:2px solid {BRAND_BLUE};background:#FFFFFF">
        ...
        <div style="font-size:13px;color:{BRAND_GREEN};font-style:italic;margin-top:2px">{BUSINESS_TAGLINE}</div>
        {subtitle_html}
    </div>
    """.strip()
