...
user_problem_statement: "Add 'Encaisser' (Collect payment) button on appointment detail screen. Always visible, opens a modal pre-filled with appointment data, allows selecting Comptant/E-transfert payment method and Printemps/Automne category, and on submit creates a Revenue + marks the appointment as paid."

backend:
  - task: "Encaisser appointment endpoint (POST /api/appointments/{id}/encaisser)"
    implemented: true
    working: true
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "NEW: POST /api/appointments/{appointment_id}/encaisser. Body: {amount (float >0, required), payment_method ('cash'|'etransfert'), category ('printemps'|'automne'), date ('YYYY-MM-DD'), description (str)}. Atomically creates a Revenue document linked to the appointment (with appointment_id field) AND sets the appointment's status='paid' + paid_at + paid_amount + paid_method + revenue_id fields. AppointmentResponse model updated to include paid_at/paid_amount/paid_method/revenue_id Optional fields. Validation: amount<=0 → 400 'Le montant doit être positif'; payment_method NOT in {cash, etransfert} → 400; category NOT in VALID_REVENUE_CATEGORIES → 400; appointment not found → 404. If date is omitted → defaults to today UTC. If description is omitted → defaults to 'Lavage de vitres - {client_address}'. Manually verified via curl: happy path returns {ok:true, revenue:{...}, status:'paid', paid_amount, paid_method}; GET /appointments/{id} after encaisser returns full new fields; revenue is queryable in /api/revenues. Tested negative cases (amount=0, invalid pm 'bitcoin', nonexistent appt) all return correct 4xx codes. Test data cleaned up. Related frontend: detail.tsx now has 'Encaisser' button + modal that calls this endpoint."
        - working: true
          agent: "testing"
          comment: "✅ 70/70 assertions PASS via /app/backend_test.py against EXPO_PUBLIC_BACKEND_URL (https://booking-hub-406.preview.emergentagent.com/api). Scope strictly limited to POST /appointments/{id}/encaisser + regression checks on AppointmentResponse and /revenues. SETUP: POST /api/appointments (title='Test Encaisser', client_name='Encaisser Test', client_email='', client_phone='5141234567', client_address='456 Rue Test', date='2026-12-20', time_slot='14:00', duration=60, price=200.00, status='upcoming') → 200 id captured. TEST 1 (happy path — full body {amount:200,payment_method:etransfert,category:printemps,date:'2026-12-20',description:'Lavage de vitres - 456 Rue Test'}): HTTP 200, response.ok=true, response.status='paid', response.paid_amount=200.0, response.paid_method='etransfert', response.paid_at is valid ISO timestamp (parses via datetime.fromisoformat), response.revenue.id UUID present, revenue.amount=200.0, revenue.category='printemps', revenue.payment_method='etransfert', revenue.appointment_id matches, revenue.date='2026-12-20', revenue.description='Lavage de vitres - 456 Rue Test'. TEST 2 (GET /{id} after encaisser): 200, status='paid', paid_at present, paid_amount=200.0, paid_method='etransfert', revenue_id matches step-1 revenue id. TEST 3 (GET /api/revenues): 200, step-1 revenue found with appointment_id matching and amount=200.0. TEST 4 (defaults — minimal body {amount:50,payment_method:cash,category:automne} on second appointment 'Test Encaisser 2' with address='789 Avenue Default'): 200, revenue.date defaults to today UTC ('2026-05-06'), revenue.description defaults to 'Lavage de vitres - 789 Avenue Default', amount=50.0, category='automne', payment_method='cash'. TEST 5 (amount=0): 400 detail contains 'montant'+'positif'. TEST 6 (amount=-50): 400 same message. TEST 7 (payment_method='bitcoin'): 400 detail='Mode de paiement: cash ou etransfert'. TEST 8 (category='hiver'): 400 detail contains 'Catégorie'+'invalide'. TEST 9 (nonexistent-xyz-uuid): 404 detail='Appointment not found'. TEST 10 (AppointmentResponse regression via GET /api/appointments?include_archived=true): paid appointment list item has all 4 new keys populated correctly (paid_at, paid_amount=200.0, paid_method='etransfert', revenue_id matches); unpaid appointment list item has all 4 keys present and all null — confirms Pydantic Optional default serialization works. CLEANUP: DELETE /api/revenues/{id} for both revenues → 200; DELETE /api/appointments/{id}/permanent for all 3 test appointments → 200; DELETE /api/clients-db/{id} for all 3 auto-linked clients → 200. Final GET /api/revenues count=12 matches pre-test baseline of 12 (zero leaks). No critical issues."

  - task: "Reschedule appointment with email notification (PUT /api/appointments/{id} with notify_client + date/time change)"
    implemented: true
    working: true
    file: "/app/backend/server.py"
    stuck_count: 0
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "ENHANCED PUT /api/appointments/{id}: AppointmentUpdate now accepts an extra non-stored 'notify_client' bool. Endpoint reads the current appointment first, detects whether 'date' or 'time_slot' actually changed (compared to stored values), strips notify_client from the $set payload, then if (changed AND notify_client AND client_email present AND resend.api_key set) sends a branded reschedule email via Resend (template shows OLD slot strikethrough + NEW slot in green block + duration + address). Email passes through inject_branding() so the Gexia360 header/logo + CASL unsubscribe footer are added. Failures of email send are caught (try/except, logged) and DO NOT fail the PUT. New helper _fmt_date_fr(d) added at module top to format YYYY-MM-DD as French long date (e.g. '25 avril 2026'). Backward compat: PUT calls without notify_client still work as before; PUT calls that don't change date/time just update other fields and skip email. Test scope: (1) PUT with notify_client=True + new date/time + client_email present → 200, appointment updated, email send attempted (in test env Resend may fail with sandbox limit but call should NOT raise); (2) PUT with notify_client=True but ONLY status change (no date/time change) → 200, NO email send; (3) PUT with notify_client=False + date/time change → 200, NO email send; (4) PUT with notify_client absent → 200, NO email send (default); (5) Verify notify_client never persists to Mongo doc; (6) Verify _fmt_date_fr handles bad input gracefully."
        - working: true
          agent: "testing"
          comment: "✅ 46/46 assertions PASS via /app/backend_test.py against EXPO_PUBLIC_BACKEND_URL. Setup: POST /api/appointments (title='Test reprog', client_email='lavagedevitreboisfranc@live.com', date='2026-12-01', time_slot='09:00', duration=60, status='upcoming') → 200, id captured. CASE 1 (date+time change + notify_client=true): PUT body {date:'2026-12-15', time_slot:'10:30', notify_client:true} → 200, response.date='2026-12-15', response.time_slot='10:30', 'notify_client' NOT in response keys (AppointmentResponse strips it correctly). Backend log line confirmed: '2026-04-25 00:27:51,397 - server - INFO - Reschedule email sent to client lavagedevitreboisfranc@live.com' — Resend email actually sent successfully (custom domain louis-philippe@lavagedevitre.org permits delivery to NOTIFY_EMAIL). CASE 2 (status='completed' + notify_client=true, no date/time change): 200, status updated to 'completed', date/time_slot UNCHANGED ('2026-12-15'/'10:30'), 'notify_client' absent from response, NO new 'Reschedule email sent' or 'Failed' log line emitted (correct: is_reschedule=false short-circuits send). CASE 3 (date change + notify_client=false): 200, date='2026-12-20', no email-attempt log entry (notify_client=False short-circuits). CASE 4 (time_slot change with no notify_client): 200, time_slot='11:00', no email-attempt log entry (default notify_client absent → False). CASE 5 (second appointment with empty client_email + notify_client=true + date/time change): 200, date='2026-12-25', time_slot='14:00', no email-attempt log entry (empty client_email short-circuits before Resend.Emails.send). CASE 6 (GET /{id}): 200, response keys=['id','title','client_name','client_email','client_phone','client_address','date','time_slot','duration_minutes','price','notes','status','created_at','assigned_to','assigned_id','assigned_color','client_photo','client_id','archived_at'] — 'notify_client' field absent ✓. CASE 7 (PUT empty body {}): 400 with detail='No fields to update' ✓. CASE 8 (PUT nonexistent uuid): 404 with detail='Appointment not found' ✓. CASE 9 (_fmt_date_fr): direct import server module → m._fmt_date_fr('2026-12-15')='15 décembre 2026' EXACT, m._fmt_date_fr('')='', m._fmt_date_fr('not-a-date')='not-a-date' (graceful fallback works). CLEANUP: DELETE /api/appointments/{id}/permanent for both test appointments → 200; DELETE /api/clients-db/{auto-linked-client-id} → 200; final GET /api/appointments?include_archived=true confirms both ids absent from list. DB left clean. No critical issues — endpoint correctly strips notify_client from $set payload (never persisted), correctly sends branded reschedule email when (date or time changed) AND notify_client=true AND client_email!='' AND resend.api_key set, gracefully short-circuits all 4 negative paths, and exception-safe try/except prevents email failures from breaking the PUT response."
...
test_plan:
  current_focus:
    - "Encaisser appointment endpoint (POST /api/appointments/{id}/encaisser)"
  stuck_tasks: []
  test_all: false
...
agent_communication:
    - agent: "main"
      message: "NEW FEATURE — Reschedule appointment endpoint enhancement. Please test PUT /api/appointments/{id} with the new notify_client flag. Setup: create an appointment with client_email='lavagedevitreboisfranc@live.com' (NOTIFY_EMAIL is allowed by Resend custom domain). Cases: (1) PUT {date:'<future>', time_slot:'10:30', notify_client:true} where date/time differ from current → expect 200, AppointmentResponse reflects new date/time, notify_client NOT in response, no exception. (2) PUT {status:'completed', notify_client:true} (no date/time change) → 200, no email logic triggered. (3) PUT {date:'<other>', notify_client:false} → 200, date updated, no email. (4) PUT without notify_client → 200, default behaviour. (5) PUT {date:'<other>', notify_client:true} on appointment WITHOUT client_email → 200, no email attempted. (6) Confirm via GET /{id} that the document does NOT contain a 'notify_client' field. (7) Verify _fmt_date_fr produces '25 avril 2026' format (can be observed in logs via reschedule send). Cleanup: DELETE /{id}/permanent on the test appointment(s). Important: Resend may log success or 'restricted-sandbox' error depending on the from-domain — both are acceptable as long as the endpoint returns 200 and no unhandled exception."
    - agent: "main"
    ...
    - agent: "testing"
      message: "✅ RECEIPT DELETION FEATURE PASS — 54/54 assertions via /app/backend_test.py against EXPO_PUBLIC_BACKEND_URL. Scope strictly limited to DELETE /api/expenses/{id}/receipt and PUT /api/expenses/{id} null-handling fix (no other endpoints touched). (A) DELETE /receipt?type=photo on expense with both receipt_photo AND receipt_pdf → 200, response {'deleted':'photo','expense':{...}} with expense.receipt_photo=null and expense.receipt_pdf preserved; persistence confirmed via GET /{id} AND GET /expenses list. (B) DELETE /receipt?type=pdf → 200, receipt_pdf=null, receipt_photo preserved, persisted. (C) DELETE /receipt?type=all → 200, both fields null, persisted; default (no type param) behaves identically with deleted='all'. (D) DELETE /receipt?type=invalid → 400, detail='Type invalide (photo|pdf|all)'. (E) DELETE /receipt on nonexistent-xyz-uuid-1234 → 404, detail='Dépense introuvable'. (F) PUT {'receipt_photo': null} on expense with photo → 200, response.receipt_photo=null, persisted via GET (bug fix verified — previously silently ignored). (G) PUT {'receipt_pdf': null} → 200, receipt_pdf=null, persisted. (H) PUT {'amount': null, 'vendor': 'New Vendor'} → 200: amount STAYS 42.0 (NOT wiped), vendor updated to 'New Vendor' — confirms the ALLOW_NULL_FIELDS whitelist only permits null on {receipt_photo, receipt_pdf, description, vendor}; also verified {'category': null, 'date': null, 'amount': 123.45} preserves category='gas' and date='2026-01-15' while updating amount. (I) PUT {'description': null} → 200, description cleared to '' (accepted behaviour — 'original' text is gone), persisted. (J) After DELETE /receipt?type=all the expense itself is STILL in GET /expenses list and GET /{id} still returns 200 — only receipt fields are nulled. CLEANUP: all 11 test expenses deleted; final GET /api/expenses count=3 (identical to pre-existing baseline of 3, no leaks). Implementation note: DELETE /receipt uses MongoDB $set with None rather than $unset (line 2900), so the fields stay present-but-null rather than being removed; both approaches produce the same JSON response (null) and equally satisfy the spec. No critical issues."
    - agent: "testing"
      message: "✅ ENCAISSER ENDPOINT PASS — 70/70 assertions via /app/backend_test.py against EXPO_PUBLIC_BACKEND_URL (https://booking-hub-406.preview.emergentagent.com/api). Scope strictly limited to POST /api/appointments/{id}/encaisser + AppointmentResponse regression + /api/revenues visibility — no other endpoints touched. SETUP created 3 test appointments + tracked auto-linked clients (3 of them). TEST 1 happy path full body: 200, ok=true, status='paid', paid_amount=200.0, paid_method='etransfert', paid_at valid ISO (parsed via datetime.fromisoformat), revenue.id UUID present, revenue.amount/category/payment_method/appointment_id/date/description all match request body exactly. TEST 2 GET /{id}: 200, status='paid', paid_at present, paid_amount=200.0, paid_method='etransfert', revenue_id matches step-1 revenue id. TEST 3 GET /api/revenues: 200, step-1 revenue found with appointment_id + amount matching. TEST 4 defaults — minimal body {amount:50,payment_method:cash,category:automne}: 200, revenue.date defaults to today UTC ('2026-05-06'), revenue.description defaults to 'Lavage de vitres - 789 Avenue Default' (client_address-based). TEST 5 amount=0: 400 detail contains 'montant'+'positif'. TEST 6 amount=-50: 400 same message. TEST 7 payment_method='bitcoin': 400 detail='Mode de paiement: cash ou etransfert'. TEST 8 category='hiver': 400 detail contains 'Catégorie'+'invalide'. TEST 9 nonexistent-xyz-uuid: 404 detail='Appointment not found' exact. TEST 10 AppointmentResponse regression: GET /appointments?include_archived=true — paid appointment list item has all 4 new keys populated (paid_at, paid_amount=200.0, paid_method='etransfert', revenue_id matches); unpaid appointment list item has all 4 keys present and null — confirms Pydantic Optional[...] default None serializes as explicit null. CLEANUP: DELETE /api/revenues/{id} × 2 → 200; DELETE /api/appointments/{id}/permanent × 3 → 200; DELETE /api/clients-db/{id} × 3 → 200. Final GET /api/revenues count=12 matches pre-test baseline of 12 (zero leaks). No critical issues — endpoint correctly atomically creates Revenue doc, marks appt as paid with all 4 new fields, defaults date to today UTC and description to 'Lavage de vitres - {client_address}' when omitted, validates amount>0 + payment_method ∈ {cash,etransfert} + category ∈ VALID_REVENUE_CATEGORIES, returns 404 for unknown appointment id."
    - agent: "testing"
      message: "✅ RESCHEDULE APPOINTMENT WITH EMAIL NOTIFICATION PASS — 46/46 assertions via /app/backend_test.py against EXPO_PUBLIC_BACKEND_URL. Scope strictly limited to PUT /api/appointments/{id} with the new notify_client flag and the helper _fmt_date_fr — no other endpoints touched. SETUP: POST /api/appointments (title='Test reprog', client_name='Test Client', client_email='lavagedevitreboisfranc@live.com', phone='5141234567', address='123 Test St', date='2026-12-01', time_slot='09:00', duration=60, status='upcoming') → 200, id captured. CASE 1 (date+time change + notify_client=true): PUT body {date:'2026-12-15',time_slot:'10:30',notify_client:true} → 200, response.date='2026-12-15', response.time_slot='10:30', notify_client field absent from response. Backend log line confirmed: '2026-04-25 00:27:51,397 - server - INFO - Reschedule email sent to client lavagedevitreboisfranc@live.com' — Resend actually delivered (custom domain louis-philippe@lavagedevitre.org permits delivery to NOTIFY_EMAIL). No exception raised. CASE 2 (status change + notify_client=true, NO date/time change): 200, status='completed', date+time UNCHANGED, no new 'Reschedule email' log entry — is_reschedule=false correctly short-circuits the send. CASE 3 (date change + notify_client=false): 200, date updated, NO email-attempt log entry. CASE 4 (time_slot change with notify_client absent): 200, time_slot updated, NO email-attempt log entry (default False). CASE 5 (second appointment with empty client_email + date/time change + notify_client=true): 200, date+time updated, NO email-attempt log entry — empty email correctly short-circuits before resend.Emails.send. CASE 6 (GET /{id}): response keys list does NOT contain 'notify_client' (Pydantic AppointmentResponse correctly strips it; field never persisted to Mongo $set either since endpoint pops it from raw dict before update). CASE 7 (PUT empty body {}): 400 detail='No fields to update' ✓. CASE 8 (PUT nonexistent uuid): 404 detail='Appointment not found' ✓. CASE 9 (_fmt_date_fr direct import): m._fmt_date_fr('2026-12-15')='15 décembre 2026' EXACT, m._fmt_date_fr('')='', m._fmt_date_fr('not-a-date')='not-a-date' (graceful fallback works). CLEANUP: DELETE /api/appointments/{id}/permanent for both test appointments → 200; auto-linked client (id=08feeac3-ad1d-4873-adc1-0c291639effc) deleted via DELETE /api/clients-db/{id} → 200; final GET /api/appointments?include_archived=true confirms BOTH ids absent from list. DB left clean. No critical issues."
    - agent: "main"
      message: "FEATURE FRONTEND-only — Added daily appointment list + duration-aware busy slots to /app/frontend/app/create.tsx (Nouveau RDV / Proposer RDV / Modifier). The screen now: (1) stores full appointment objects for the selected date in state dayAppointments (sorted by time_slot), (2) computes busySlots so an appointment with duration 60/90/120/etc minutes blocks ALL overlapping 30-min sub-slots (not just the start slot), (3) renders a yellow card list below the slot grid titled '📋 Rendez-vous du jour (N)' showing each existing booking's time + client_name + duration — identical visual to /app/frontend/app/reschedule.tsx. Verified visually via screenshot on http://localhost:3000/create: selecting 2026-04-27 displays 'Brigitte Roy 09:30 (120min)' + 'Marjolaine Lachance 13:30 (120min)' and barre rouge on 09:30/10:00/10:30/11:00 + 13:30/14:00/14:30/15:00. No backend changes; no test agent invocation needed."
    - agent: "main"
      message: "FEATURE FRONTEND-only — Added reusable client autocomplete /app/frontend/src/components/ClientAutocomplete.tsx and wired it into 3 screens. Component shows a dropdown of matching clients (by name/phone/email) as the user types ≥2 chars, with module-level shared cache so /api/clients-db?limit=2000 fires only once per session (also exposes invalidateClientCache() so /app/frontend/app/clients-db.tsx can wipe the cache after add/edit/delete/import — fetchClients() now calls it). Applied to: /app/frontend/app/create.tsx (Nouveau RDV — also prefills phone/email/address when a client is picked), /app/frontend/app/estimate.tsx (Estimation form — prefills email), /app/frontend/app/revenues.tsx (Revenue form — fills name only). Verified visually: typing 'bri' in each screen yields the 4-row dropdown (Brigitte Roy / Melanie Briere / Sabrina Corsi / Sabrina Guarascio); tapping Sabrina Guarascio in create.tsx populates phone=5145184993, email=S_guarascio@yahoo.com, address='3361 rue des outardes, Ville Saint-Laurent'. No backend changes; no test agent invocation needed."
    - agent: "main"
      message: "FEATURE BACKEND — (1) Rewrote sms_body in /app/backend/server.py POST /api/appointments/{id}/send-client-confirmation. New template: 'Bonjour {first_name} 👋\\n\\nNous avons réservé une place pour votre lavage de vitres :\\n\\n📅 {weekday} {d} {month} {y}\\n🕧 {h}h{mm}\\n📍 {addr}\\n\\n✅ Cliquez ici pour accepter ou modifier votre rendez-vous :\\n{portal_url}\\n\\nAu plaisir de vous servir!\\n\\n— Louis-Philippe\\nLavage de Vitres Bois-Franc\\nService propulsé par Gexia360'. Added French weekday calc (Lundi…Dimanche via _date.weekday()), French time format (12h30 instead of 12:30), and first_name extraction (split on space). (2) Added e-transfer payment block inside /api/invoice/{id} HTML right after the totals table: green card '💳 Paiement par e-transfert' with mailto link to lavagedevitreboisfranc@live.com and note 'Aucune question secrète requise (dépôt automatique)'. Verified with curl: sms_body now reads 'Bonjour Geneviève 👋 … 📅 Mardi 22 septembre 2026 🕧 9h30 …'. Invoice HTML verified visually — green payment card renders correctly between totals and promo banner. No frontend changes; backend hot-reloaded successfully."
